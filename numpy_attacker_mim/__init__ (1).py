...
class Attacker(BatchAttack):
    ...
    def batch_attack(self, xs, ys=None, ys_target=None):
        ...
        xs_flatten_shape = (self.batch_size, np.prod(self.model.x_shape))
        # print(xs_flatten_shape)
        decay_factor_var = 1.0
        g_var = np.zeros(xs_flatten_shape)
        xs_adv = xs
        ...
        for i in range(self.iteration): 
            ...
            grad = self._session.run(self.grad, feed_dict={self.xs_ph: xs_adv, self.ys_ph: ys})
            grad_flatten = grad.reshape(xs_flatten_shape)
            grad_l1 = np.sum(np.abs(grad_flatten), axis=1)
            # print(np.shape(grad_l1))
            g_next = decay_factor_var * g_var + grad_flatten / np.expand_dims(grad_l1, 1)
            g_var = g_next.copy()
            ...
            grad_new = g_next.reshape(self.batch_size, *self.model.x_shape)
            grad_sign = np.sign(grad_new)
